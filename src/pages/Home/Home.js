...
const Home = () => {
  ...
  const updateDisplayedProducts = () => {
    ...
    const start = (currentPage - 1) * pageSize;
    const end = start + pageSize;
    setDisplayedProducts(filteredAndSortedProducts.slice(start, end));
  };
  ...
  const applyFiltersAndSort = (products) => {
    let filtered = products.filter((product) => {
      ...
      return meetsPositiveFilters && meetsNegativeFilters && matchesSearchTerm;
    });
  ...
  // Sortierlogik
  sortConfigs.forEach((config) => {
    filtered = filtered.sort(/* Ihre Sortierfunktion */);
  });
